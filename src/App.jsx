...
function SmartImage({image,className,...props}) {
...
function Rail({images,label}){
  ...
  return <div className="carousel" role="region" aria-roledescription="carousel" aria-label={label}>
    <div className="rail" ref={railRef}>
      <div className="rail-track">{images.map((image,i)=><div className="rail-slide" role="group" aria-roledescription="slide" aria-label={`${i+1} of ${images.length}`} key={image.base}><SmartImage image={image} loading="lazy" decoding="async" /></div>)}</div>
    </div>
...
export function App(){
  ...
  return <main>
    ...
    <section className="about content-block"><SmartImage image={images.people.photographer} loading="lazy" decoding="async"/><div><p className="eyebrow">Meet the photographer</p><h2>Hi! I'm Madhu</h2><p>I’m the founder and lead photographer behind ijós Moments.</p><p>I’ve been photographing people and stories since 2013. My approach is simple: help you feel comfortable, blend into the background when the moments matter, and capture your day as naturally as it unfolds.</p><p>The best compliment I can receive is hearing that it felt like having a friend with a camera, not a photographer taking over the day.</p></div></section>

    <section className="service left content-block"><div><h2>Wedding Day</h2><p>From intimate solemnisations to multi-day wedding celebrations, we create photos and videos that let you relive the people, emotions and moments that made the day yours.</p></div><Rail images={images.galleries.weddingDay} label="Wedding day gallery"/></section>
    <section className="service right content-block"><Rail images={images.galleries.preWedding} label="Pre-wedding gallery"/><div><h2>Pre-wedding shoots</h2><p>Meaningful pre wedding photography that captures this chapter of your story with honesty, warmth and timeless style.</p></div></section>
    <section className="service left content-block"><div><h2>Proposal</h2><p>Planning the perfect proposal? We’ll work behind the scenes to ensure every meaningful moment is beautifully and discreetly captured.</p></div><Rail images={images.galleries.proposal} label="Proposal gallery"/></section>
